...
public final class ChandelierExit implements AutoCloseable {
     ...
     */
    public ChandelierExitOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_CHANDELIER_EXIT_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            ChandelierExitOutput[] out = new ChandelierExitOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new ChandelierExitOutput(
