...
public final class FractalChaosBands implements AutoCloseable {
     ...
     */
    public FractalChaosBandsOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_FRACTAL_CHAOS_BANDS_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            FractalChaosBandsOutput[] out = new FractalChaosBandsOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new FractalChaosBandsOutput(
