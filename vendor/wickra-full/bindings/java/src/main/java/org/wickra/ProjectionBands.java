...
public final class ProjectionBands implements AutoCloseable {
     ...
     */
    public ProjectionBandsOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_PROJECTION_BANDS_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            ProjectionBandsOutput[] out = new ProjectionBandsOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new ProjectionBandsOutput(
