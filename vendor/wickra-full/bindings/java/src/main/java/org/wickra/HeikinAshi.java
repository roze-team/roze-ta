...
public final class HeikinAshi implements AutoCloseable {
     ...
     */
    public HeikinAshiOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_HEIKIN_ASHI_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            HeikinAshiOutput[] out = new HeikinAshiOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new HeikinAshiOutput(
