...
public final class Keltner implements AutoCloseable {
     ...
     */
    public KeltnerOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_KELTNER_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            KeltnerOutput[] out = new KeltnerOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new KeltnerOutput(
