...
public final class VolatilityCone implements AutoCloseable {
     ...
     */
    public VolatilityConeOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_VOLATILITY_CONE_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            VolatilityConeOutput[] out = new VolatilityConeOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new VolatilityConeOutput(
