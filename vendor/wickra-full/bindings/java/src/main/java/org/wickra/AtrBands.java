...
public final class AtrBands implements AutoCloseable {
     ...
     */
    public AtrBandsOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_ATR_BANDS_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            AtrBandsOutput[] out = new AtrBandsOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new AtrBandsOutput(
