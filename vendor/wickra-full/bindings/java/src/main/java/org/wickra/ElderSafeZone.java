...
public final class ElderSafeZone implements AutoCloseable {
     ...
     */
    public ElderSafeZoneOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_ELDER_SAFE_ZONE_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            ElderSafeZoneOutput[] out = new ElderSafeZoneOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new ElderSafeZoneOutput(
