...
public final class HurstChannel implements AutoCloseable {
     ...
     */
    public HurstChannelOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_HURST_CHANNEL_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            HurstChannelOutput[] out = new HurstChannelOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new HurstChannelOutput(
