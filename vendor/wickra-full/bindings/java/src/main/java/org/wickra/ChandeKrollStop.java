...
public final class ChandeKrollStop implements AutoCloseable {
     ...
     */
    public ChandeKrollStopOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_CHANDE_KROLL_STOP_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            ChandeKrollStopOutput[] out = new ChandeKrollStopOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new ChandeKrollStopOutput(
