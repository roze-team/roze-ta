...
public final class Mama implements AutoCloseable {
     ...
     */
    public MamaOutput[] batch(double[] input) {
        int n = input.length;
        try (Arena a = Arena.ofConfined()) {
            MemorySegment inputSeg = a.allocateFrom(JAVA_DOUBLE, input);
            MemorySegment outSeg = a.allocate(16L * n);
            NativeMethods.WICKRA_MAMA_BATCH.invokeExact(handle(), inputSeg, outSeg, (long) n);
            MamaOutput[] out = new MamaOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new MamaOutput(
