...
public final class Adx implements AutoCloseable {
     ...
     */
    public AdxOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_ADX_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            AdxOutput[] out = new AdxOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new AdxOutput(
