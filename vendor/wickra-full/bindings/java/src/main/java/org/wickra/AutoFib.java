...
public final class AutoFib implements AutoCloseable {
     ...
     */
    public AutoFibOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_AUTO_FIB_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            AutoFibOutput[] out = new AutoFibOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new AutoFibOutput(
