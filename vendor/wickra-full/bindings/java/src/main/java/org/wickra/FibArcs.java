...
public final class FibArcs implements AutoCloseable {
     ...
     */
    public FibArcsOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_FIB_ARCS_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            FibArcsOutput[] out = new FibArcsOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new FibArcsOutput(
