...
public final class Camarilla implements AutoCloseable {
     ...
     */
    public CamarillaPivotsOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            CamarillaPivotsOutput[] out = new CamarillaPivotsOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new CamarillaPivotsOutput(
