...
public final class TdMovingAverage implements AutoCloseable {
     ...
     */
    public TdMovingAverageOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_TD_MOVING_AVERAGE_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            TdMovingAverageOutput[] out = new TdMovingAverageOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new TdMovingAverageOutput(
