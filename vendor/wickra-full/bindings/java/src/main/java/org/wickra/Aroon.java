...
public final class Aroon implements AutoCloseable {
     ...
     */
    public AroonOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_AROON_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            AroonOutput[] out = new AroonOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new AroonOutput(
