...
public final class Stochastic implements AutoCloseable {
     ...
     */
    public StochasticOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_STOCHASTIC_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            StochasticOutput[] out = new StochasticOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new StochasticOutput(
