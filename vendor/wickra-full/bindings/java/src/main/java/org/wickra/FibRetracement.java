...
public final class FibRetracement implements AutoCloseable {
     ...
     */
    public FibRetracementOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_FIB_RETRACEMENT_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            FibRetracementOutput[] out = new FibRetracementOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new FibRetracementOutput(
