...
public final class HighLowVolumeNodes implements AutoCloseable {
     ...
     */
    public HighLowVolumeNodesOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_HIGH_LOW_VOLUME_NODES_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            HighLowVolumeNodesOutput[] out = new HighLowVolumeNodesOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new HighLowVolumeNodesOutput(
