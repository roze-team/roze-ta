...
public final class Ichimoku implements AutoCloseable {
     ...
     */
    public IchimokuOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_ICHIMOKU_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            IchimokuOutput[] out = new IchimokuOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new IchimokuOutput(
