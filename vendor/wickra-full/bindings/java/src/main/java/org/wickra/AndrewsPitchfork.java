...
public final class AndrewsPitchfork implements AutoCloseable {
     ...
     */
    public AndrewsPitchforkOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_ANDREWS_PITCHFORK_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            AndrewsPitchforkOutput[] out = new AndrewsPitchforkOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new AndrewsPitchforkOutput(
