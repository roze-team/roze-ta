...
public final class SmoothedHeikinAshi implements AutoCloseable {
     ...
     */
    public SmoothedHeikinAshiOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_SMOOTHED_HEIKIN_ASHI_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            SmoothedHeikinAshiOutput[] out = new SmoothedHeikinAshiOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new SmoothedHeikinAshiOutput(
