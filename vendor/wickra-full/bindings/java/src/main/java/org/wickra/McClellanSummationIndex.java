...
public final class McClellanSummationIndex implements AutoCloseable {
    ...
    public double[] batch(double[] change, double[] volume, boolean[] newHigh, boolean[] newLow, boolean[] aboveMa, boolean[] onBuySignal, int members, long[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_MC_CLELLAN_SUMMATION_INDEX_BATCH.invokeExact(handle(), changeSeg, volumeSeg, newHighSeg, newLowSeg, aboveMaSeg, onBuySignalSeg, (long) members, timestampSeg, outSeg, (long) n);
