...
public final class WaveTrend implements AutoCloseable {
     ...
     */
    public WaveTrendOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_WAVE_TREND_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            WaveTrendOutput[] out = new WaveTrendOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new WaveTrendOutput(
