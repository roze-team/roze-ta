...
public final class CandleVolume implements AutoCloseable {
     ...
     */
    public CandleVolumeOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_CANDLE_VOLUME_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            CandleVolumeOutput[] out = new CandleVolumeOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new CandleVolumeOutput(
