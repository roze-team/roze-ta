...
public final class VolumeWeightedMacd implements AutoCloseable {
     ...
     */
    public VolumeWeightedMacdOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_VOLUME_WEIGHTED_MACD_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            VolumeWeightedMacdOutput[] out = new VolumeWeightedMacdOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new VolumeWeightedMacdOutput(
