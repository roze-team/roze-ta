...
public final class RunBars implements AutoCloseable {
     ...
     */
    public RunBar[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            long total = (long) NativeMethods.WICKRA_RUN_BARS_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, (long) n);
            if (total <= 0) {
                return new RunBar[0];
            }
            MemorySegment buf = a.allocate(48L * total);
            long drained = (long) NativeMethods.WICKRA_RUN_BARS_DRAIN.invokeExact(handle(), buf, total);
            RunBar[] result = new RunBar[(int) drained];
            for (int i = 0; i < drained; i++) {
                long b = (long) i * 48L;
                result[i] = new RunBar(
                    ...
                    (double) buf.get(JAVA_LONG, b + 32L),
