...
public final class KagiBars implements AutoCloseable {
     ...
     */
    public KagiBar[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            long total = (long) NativeMethods.WICKRA_KAGI_BARS_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, (long) n);
            if (total <= 0) {
                return new KagiBar[0];
            }
            MemorySegment buf = a.allocate(24L * total);
            long drained = (long) NativeMethods.WICKRA_KAGI_BARS_DRAIN.invokeExact(handle(), buf, total);
            KagiBar[] result = new KagiBar[(int) drained];
            for (int i = 0; i < drained; i++) {
                long b = (long) i * 24L;
                result[i] = new KagiBar(
