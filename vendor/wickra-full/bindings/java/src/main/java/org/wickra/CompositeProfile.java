...
public final class CompositeProfile implements AutoCloseable {
     ...
     */
    public CompositeProfileOutput[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        int n = open.length;
        if (high.length != n) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_COMPOSITE_PROFILE_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
            CompositeProfileOutput[] out = new CompositeProfileOutput[n];
            for (int i = 0; i < n; i++) {
                out[i] = new CompositeProfileOutput(
