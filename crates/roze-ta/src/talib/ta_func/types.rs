...
/// [`ParamHolder::call`](crate::talib::abstract_api::ParamHolder::call). A valid range
...
///
/// ```
/// use roze_ta::talib::{Core, FuncUnstId};
///
/// let core = Core::builder()
///     .unstable_period(FuncUnstId::EMA, 10)
///     .build()?;
/// # Ok::<(), roze_ta::talib::RetCode>(())
/// ```
///
...
impl CoreBuilder {
    ...
    ///
    /// ```
    /// use roze_ta::talib::{CandleSetting, CandleSettingType, Core, RangeType};
    ///
...
mod tests {
    ...
    fn a_short_history_open_reports_insufficient_history() {
        ...
        assert!(
            lookback > 0,
            "the probe needs a function that consumes bars"
        );
        ...
        assert_eq!(
            core.sma_open(&just_enough, 0).err(),
    ...
    fn builder_unstable_period_wildcard_sets_every_function() {
        let core = Core::builder()
            .unstable_period(FuncUnstId::ALL, 7)
            .build()
            .unwrap();
    ...
    fn get_unstable_period_accepts_the_last_real_function() {
        ...
        let core = Core::builder()
            .unstable_period(FuncUnstId::ALL, 4)
            .build()
            .unwrap();
        for id in [
            FuncUnstId::ADX,
    ...
    fn unstable_period_bound_is_a_bound_not_an_off_by_one() {
        ...
        let core = Core::builder()
            .unstable_period(FuncUnstId::EMA, ceiling)
            .build()
            .unwrap();
    ...
    fn to_builder_round_trip_is_total() {
        ...
        let ceiling = u32::try_from(Core::MAX_INDEX).unwrap();
        let core = Core::builder()
            .unstable_period(FuncUnstId::ALL, ceiling)
            .build()
            .unwrap();
        let again = core
            .to_builder()
            .build()
            .expect("a built Core must always rebuild");
    ...
    fn builder_candle_setting_overrides_one_leaves_rest() {
        ...
        let core = Core::builder()
            ...
            .unwrap();
        assert_eq!(
            core.candle_settings.body_long.range_type,
            RangeType::Shadows
        );
    ...
    fn restore_candle_default_derives_a_new_core_and_leaves_the_old_one_alone() {
        ...
        assert_eq!(
            derived.candle_settings.body_long,
            CandleSetting {
                range_type: RangeType::RealBody,
                avg_period: 10,
                factor: 1.0
    ...
    fn a_rejected_candle_setting_writes_nothing() {
        ...
        let defaults = CandleSettings::default_settings();
        assert_eq!(
            builder.candle_settings.body_long.avg_period,
            defaults.body_long.avg_period
        );
        assert_eq!(builder.candle_settings.equal.factor, defaults.equal.factor);
        assert_eq!(
            builder.candle_settings.body_doji.range_type,
            defaults.body_doji.range_type
        );
    }
    ...
    fn candle_setting_accepts_the_ceilings() {
        ...
        assert_eq!(
            core.candle_settings.body_doji.avg_period,
            i32::try_from(Core::MAX_INDEX).unwrap()
        );
        assert_eq!(
            core.candle_settings.body_doji.range_type,
            RangeType::Shadows
    ...
    fn accepted_candle_settings_keep_the_lookback_and_the_call_in_step() {
        ...
        for avg_period in [0, 1, 5, 39, 40, 100] {
            ...
            assert!(
                lookback <= Core::MAX_INDEX,
                "avg_period {avg_period} gave lookback {lookback}"
            );
    ...
    fn candle_setting_flows_into_computation() {
        ...
        assert_ne!(
            default_out, tuned_out,
            "candle_setting must change CDLDOJI output"
        );
    }
    ...
    fn to_builder_round_trips_and_leaves_original_untouched() {
        ...
        let derived = original
            .to_builder()
            .unstable_period(FuncUnstId::EMA, 9)
            .build()
            .unwrap();
        ...
        assert_eq!(
            derived.candle_settings.body_long.range_type,
            RangeType::Shadows
        );
    }
    ...
    fn shared_core_runs_concurrent_batches() {
        ...
        let core = Arc::new(
            Core::builder()
                .unstable_period(FuncUnstId::EMA, 2)
                .build()
                .unwrap(),
        );
        ...
        for h in handles {
            assert_eq!(
                h.join().unwrap(),
                expected,
                "concurrent shared-Core calls must agree"
            );
        }
    }
}
