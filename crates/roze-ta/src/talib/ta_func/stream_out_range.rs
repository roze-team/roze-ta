...
use crate::talib::ta_func::types::{Core, OutRange, RetCode};
...
fn every_tier_reports_the_batch_range() {
    ...
    // Dual-mode tier.
    let batch = core
        .MINUS_DI(0, N - 1, &high, &low, &close, 14, &mut out0)
        .expect("batch MINUS_DI");
    range_tracks_batch(
        "MINUS_DI (dual-mode)",
        core.MINUS_DI_Lookback(14).expect("valid params"),
        batch,
        |w| {
            core.minus_di_open(&high[..w], &low[..w], &close[..w], 14)
                .map(|(h, _)| h)
        },
        |h| h.out_range(),
        |h, t| {
    ...
    let batch = core
        .MA(0, N - 1, &close, 14, MAType::EMA, &mut out0)
        .expect("batch MA");
    range_tracks_batch(
        "MA (dispatch, EMA arm)",
    ...
    let batch = core
        .MA(0, N - 1, &close, 1, MAType::SMA, &mut out0)
        .expect("batch MA(1)");
    range_tracks_batch(
    ...
    range_tracks_batch(
        ...
        batch,
        |w| {
            core.mavp_open(&close[..w], &periods[..w], 2, 30, MAType::SMA)
                .map(|(h, _)| h)
        },
        |h| h.out_range(),
        |h, t| {
    ...
    range_tracks_batch(
        ...
        batch,
        |w| {
            core.bbands_open(&close[..w], 20, 2.0, 2.0, MAType::SMA)
                .map(|(h, _)| h)
        },
        |h| h.out_range(),
        |h, t| {
...
fn open_and_fill_agrees_with_the_handle_it_returns() {
    ...
    let br = core
        .SMA(0, N - 1, &close, 14, &mut batch)
        .expect("batch SMA");
    let (h, fr) = core
        .sma_open_and_fill(&close, 14, &mut out)
        .expect("openAndFill");
    assert_eq!(fr, br, "the returned range is the batch range");
    assert_eq!(
        h.out_range(),
        fr,
        "the handle reports the same pair it returned"
    );
...
fn a_clone_carries_the_range_and_then_diverges() {
    ...
    assert_eq!(
        b.out_range(),
        a.out_range(),
        "a clone carries the range verbatim"
    );
    b.update(close[N - 1]).expect("finite bar");
    assert_eq!(
        b.out_range(),
        OutRange {
            beg_idx: a.out_range().beg_idx,
            count: a.out_range().count + 1
        },
...
fn a_rejected_bar_costs_nothing_and_the_caller_chooses() {
    ...
    for bad in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        let held = s.value();
        assert!(
            matches!(s.peek(bad), Err(RetCode::BadParam)),
            "a non-finite bar is rejected"
        );
        assert_eq!(
            s.out_range(),
            before,
            "peek counts nothing, rejected or not"
        );

        assert!(
            matches!(s.update(bad), Err(RetCode::BadParam)),
            "a non-finite bar is rejected"
        );
        assert_eq!(
            s.out_range(),
            before,
            "a rejected update counts nothing either"
        );
        assert_eq!(
            s.value().to_bits(),
            held.to_bits(),
    ...
    assert_eq!(
        got.to_bits(),
        want.to_bits(),
        "the rejections left nothing in the state"
    );
    assert_eq!(
        s.out_range(),
        ctl.out_range(),
...
fn advance_saturates_at_max_index() {
    ...
    while s.out_range().count < Core::MAX_INDEX {
        ...
        spins += 1;
        assert!(
            spins <= Core::MAX_INDEX,
            "advance stopped moving the count short of MAX_INDEX"
        );
    }
...
fn an_anchor_past_the_history_is_insufficient_history() {
    ...
    assert!(
        matches!(
            core.ma_open_internal(&close[..10], 30, 1, MAType::SMA),
    ...
    assert!(
        matches!(
            core.ma_open_internal(&close[..10], 30, 5, MAType::DISABLED),
            Err(RetCode::InsufficientHistory)
        ),
    ...
    assert!(
        matches!(
            core.ema_open_internal(&close[..10], 30, 1),
            Err(RetCode::InsufficientHistory)
        ),
    ...
    assert!(
        matches!(
            core.mavp_open(&close[..10], &periods[..10], 1, 30, MAType::SMA),
            Err(RetCode::InsufficientHistory)
        ),
    ...
    assert_eq!(
        lb, 0,
        "the identity arm's lookback is 0, which is what makes startIdx the anchor"
    );
    let (h, _) = core
        .ma_open_internal(&close, 5, 1, MAType::SMA)
...
fn an_anchor_past_the_history_is_rejected_by_bodies_that_do_not_check_it() {
    ...
    assert!(
        core.ad_open_internal(&high[..H], &low[..H], &close[..H], &volume[..H], H - 1)
            .is_ok(),
