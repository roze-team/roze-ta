// Maintained TA-Lib derivative; BSD-3-Clause. See docs/patches/talib-rust-migration.md.
...
use crate::talib::ta_func::types::{Core, RetCode};
...
fn update_and_peek_reject_a_non_finite_bar_without_moving_the_handle() {
    ...
    for &bad in &BAD {
        ...
        assert!(
            probed(&mut rejects, is_bad_param(&sa.update(bad))),
            "SMA.update accepted {bad}"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&sa.peek(bad))),
            "SMA.peek accepted {bad}"
        );
        assert!(
            probed(
                &mut states,
                (sa.update(nc).unwrap().to_bits() == sb.update(nc).unwrap().to_bits())
            ),
        ...
        assert!(
            probed(&mut rejects, is_bad_param(&da.update(bad, nl, nc))),
            "MINUS_DI.update(high)"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&da.update(nh, bad, nc))),
            "MINUS_DI.update(low)"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&da.update(nh, nl, bad))),
            "MINUS_DI.update(close)"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&da.peek(bad, nl, nc))),
            "MINUS_DI.peek(high)"
        );
        assert!(
            probed(
                &mut states,
                da.update(nh, nl, nc).unwrap().to_bits()
        ...
        assert!(
            probed(&mut rejects, is_bad_param(&ma.update(bad))),
            "MA.update accepted {bad}"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&ma.peek(bad))),
            "MA.peek accepted {bad}"
        );
        assert!(
            probed(
                &mut states,
                (ma.update(nc).unwrap().to_bits() == mb.update(nc).unwrap().to_bits())
            ),
        ...
        assert!(
            probed(&mut rejects, is_bad_param(&mi.update(bad))),
            "MA(identity).update"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&mi.peek(bad))),
            "MA(identity).peek"
        );
        ...
        assert!(
            probed(&mut rejects, is_bad_param(&va.update(bad, np))),
            "MAVP.update(real)"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&va.update(nc, bad))),
            "MAVP.update(period)"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&va.peek(nc, bad))),
            "MAVP.peek(period)"
        );
        assert!(
            probed(
                &mut states,
                va.update(nc, np).unwrap().to_bits() == vb.update(nc, np).unwrap().to_bits()
            ),
        ...
        assert!(
            probed(&mut rejects, is_bad_param(&ba.update(bad))),
            "BBANDS.update accepted {bad}"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&ba.peek(bad))),
            "BBANDS.peek accepted {bad}"
        );
        ...
        assert!(
            probed(
                &mut states,
                (au.to_bits(), am.to_bits(), al.to_bits())
        ...
        let (mut kb, _) = core
            ...
            .unwrap();
        assert!(
            probed(&mut rejects, is_bad_param(&ka.update(bad, nl, nc))),
            "STOCH.update"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&ka.peek(nh, bad, nc))),
            "STOCH.peek"
        );
        ...
        assert!(
            probed(
                &mut states,
                (kak.to_bits(), kad.to_bits()) == (kbk.to_bits(), kbd.to_bits())
            ),
        ...
        assert!(
            probed(&mut rejects, is_bad_param(&ja.update(bad, nh, nl, nc))),
            "CDLDOJI.update(open)"
        );
        assert!(
            probed(&mut rejects, is_bad_param(&ja.peek(no, nh, nl, bad))),
            "CDLDOJI.peek(close)"
        );
        assert!(
            probed(
                &mut states,
                ja.update(no, nh, nl, nc).unwrap() == jb.update(no, nh, nl, nc).unwrap()
            ),
    ...
    assert!(
        states >= 21,
        "the handle-unchanged sweep ran {states} compares"
    );
}
